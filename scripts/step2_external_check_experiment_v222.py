# -*- coding: utf-8 -*-
"""
Step2 External Check Experiment v2.2.2

修复 v2.2.1 的业务验收问题 - A2 没有产生 tech_barrier / commercialization caution

核心修改：
1. 增加"硬科技/汽车项目强制 caution 规则"
2. 对于命中硬科技/汽车关键词的项目，强制要求 tech_barrier + commercialization caution
3. caution 含义 = "Step1 判断是对的，但外部逻辑要求进一步验证"

输入: step1_v1_2_1.json, project_structure.json, bp_text
输出: step2_external_check_v2_2_2.json, schema_validation.json,
...
SCHEMA_VERSION = "step2_external_check_v2_2_2"
...
# Hard tech / automotive keywords
HARD_TECH_KEYWORDS = [
    "自动驾驶", "线控底盘", "轮毂电机", "汽车零部件", "电驱动系统",
    "IGBT", "SiC", "碳化硅", "智能底盘", "车规", "底盘技术",
    "autonomous_driving", "chassis", "ev_drive", "power_module",
    "autonomous driving", "autonomous", "driving system", "motor",
    "automotive", "vehicle"
]


# ============================================================
# Hard Tech Detection & Project Type
# ============================================================

SYSTEM_INTEGRATION_KEYWORDS = [
    "系统集成", "集成商", "集成", "改装", "代运营", "运营服务",
    "模块组装", "组装", "定制化", "解决方案提供商",
    "system integration", "integrator", "module assembly", "customization"
]

TECH_RD_KEYWORDS = [
    "技术研发", "底层技术", "核心硬件", "芯片", "半导体",
    "控制器", "算法", "软件定义", "AI模型",
    "tech R&D", "core hardware", "chip", "algorithm"
]

def get_project_type(step1_data: dict, project_structure: dict) -> str:
    """
    Detect project type for caution rules:
    - 'tech_rd': technology R&D / foundational tech / core hardware
    - 'system_integration': system integration / operations / project delivery
    - 'mixed': unclear, treat as tech_rd
    """
    combined = json.dumps({
        "step1": step1_data,
    ...
    si_count = sum(1 for kw in SYSTEM_INTEGRATION_KEYWORDS if kw.lower() in combined)
    rd_count = sum(1 for kw in TECH_RD_KEYWORDS if kw.lower() in combined)

    # Also check company_essence for "技术研发型" / "底层技术突破型"
    ce_raw = step1_data.get("company_essence", "")
    company_essence = ce_raw if isinstance(ce_raw, str) else json.dumps(ce_raw, ensure_ascii=False)
    kj_raw = step1_data.get("key_judgement", {})
    key_judgement = kj_raw.get("main_thesis", "") if isinstance(kj_raw, dict) else str(kj_raw)

    # If company_essence explicitly says "系统集成/运营/项目交付型"
    ce_str = company_essence if isinstance(company_essence, str) else json.dumps(company_essence, ensure_ascii=False)
    if any(kw in ce_str.lower() or kw in key_judgement for kw in SYSTEM_INTEGRATION_KEYWORDS):
        return "system_integration"

    if si_count > rd_count * 1.5:
        return "system_integration"
    elif rd_count > si_count:
        return "tech_rd"
    else:
        return "mixed"


def is_hardtech_project(step1_data: dict, project_structure: dict) -> tuple:
    """
    Detect if project is hard tech / automotive.
    Returns (is_hardtech: bool, matched_keywords: list)
    """
    matched = []

    # Check project_structure.industry_tags
    ...
    for tag in industry_tags:
        if isinstance(tag, dict):
            ...
            for keyword in HARD_TECH_KEYWORDS:
                if keyword.lower() in label or keyword.lower() in tag_name:
                    matched.append(f"industry_tag:{keyword}")
                    break

    # Check business_lines
    business_lines = project_structure.get("business_lines", [])
    for bl in business_lines:
        if isinstance(bl, dict):
            name = bl.get("name", "").lower()
            for keyword in HARD_TECH_KEYWORDS:
                if keyword.lower() in name:
                    matched.append(f"business_line:{keyword}")
                    break

    # Check risk_buckets
    risk_buckets = project_structure.get("risk_buckets", [])
    for rb in risk_buckets:
        if isinstance(rb, dict):
            bucket_name = rb.get("bucket_name", "").lower()
            bucket_key = rb.get("bucket_key", "").lower()
            if "hardtech" in bucket_key or "hardtech" in bucket_name:
                matched.append(f"bucket:{bucket_key}")

    # Check business_model_hypotheses
    hypotheses = project_structure.get("business_model_hypotheses", [])
    for h in hypotheses:
        if isinstance(h, dict):
            bk = h.get("bucket_key", "").lower()
            if "hardtech" in bk:
                matched.append(f"hypothesis:{bk}")

    # Check step1 key_judgement and red_flags
    step1_text = json.dumps(step1_data, ensure_ascii=False).lower()
    for keyword in HARD_TECH_KEYWORDS:
        if keyword.lower() in step1_text:
            matched.append(f"step1:{keyword}")
...
def build_step2_prompt(
    ...
    """Build Step2 v2.2.2 system and user prompts"""

    # Detect hard tech project
    is_hardtech, matched_keywords = is_hardtech_project(step1_data, project_structure)
...
"""
    ...
    # Detect project type for differentiated caution rules
    project_type = get_project_type(step1_data, project_structure) if is_hardtech else "none"

    # Hard tech mandatory caution rules (differentiated by project type)
    hardtech_rules = ""
    if is_hardtech:
        if project_type in ("tech_rd", "mixed"):
            hardtech_rules = f"""[MANDATORY CAUTION RULE - HARD TECH / AUTOMOTIVE PROJECT]
This project matches hard tech / automotive keywords: {', '.join(matched_keywords)}
Project type: {project_type} (技术研发/底层技术/核心硬件替代型)
For such projects, you MUST include the following in step1_external_check.checks:

1. tech_barrier caution: At least 1 check with bucket_key="tech_barrier" AND verdict="caution"
   - Content: Step1's cautious judgment is correct. External logic requires further verification
     on: engineering validation, automotive-grade certification, reliability testing,
     safety redundancy, supply chain verification.
   - "caution" does NOT mean contradicting Step1. It means: "Step1 is right, but external
     logic demands specific evidence."

2. commercialization caution: At least 1 check with bucket_key="commercialization" AND verdict="caution"
   - Content: Step1's cautious judgment is correct. External logic requires further verification
     on: mass production readiness, OEM qualification, supply chain industrialization,
     production SOP, automotive certification timeline.
   - "caution" does NOT mean contradicting Step1. It means: "Step1 is right, but external
     logic demands specific evidence."

UNLESS Step1 already provides explicit evidence of: third-party testing, OEM qualification,
audited revenue, or automotive-grade certification.

These caution checks are MANDATORY. Do not skip them even if Step1 seems correct.
"""
        elif project_type == "system_integration":
            hardtech_rules = f"""[MANDATORY CAUTION RULE - HARD TECH / AUTOMOTIVE PROJECT]
This project matches hard tech / automotive keywords: {', '.join(matched_keywords)}
Project type: {project_type} (系统集成/运营/项目交付型)
For such projects, you MUST include the following in step1_external_check.checks:

1. commercialization caution: At least 1 check with bucket_key="commercialization" AND verdict="caution"
   - Content: Step1's cautious judgment is correct. External logic requires further verification
     on: mass production readiness, OEM qualification, supply chain industrialization,
     production SOP, automotive certification timeline, customer payment track record.
   - "caution" does NOT mean contradicting Step1. It means: "Step1 is right, but external
     logic demands specific evidence."

tech_barrier checks may have verdict="support" if the project uses industry-common technology.
However, if the integration solution itself has undisclosed technical complexity or safety implications,
a tech_barrier caution is still required.
...
- schema_version: "step2_external_check_v2_2_2" (必须包含此字段)
...
- 如果是硬科技/汽车项目，必须包含 tech_barrier caution 和 commercialization caution
""" if retry else ""
...
"""
    ...
    hardtech_system_note = """
IMPORTANT - MANDATORY CAUTION: This is a hard tech / automotive project. You MUST include at least 1 tech_barrier caution and 1 commercialization caution in step1_external_check.checks. 'caution' means Step1 is correct but external logic requires further specific evidence.""" if is_hardtech else ""

    system_prompt = f"""You are an expert investment analyst. Output valid JSON only.

CRITICAL: You must output a complete JSON object with ALL these fields:
1. schema_version: "step2_external_check_v2_2_2"
...
# ============================================================
# Schema Validation v2.2.2
# ============================================================

def validate_step2_schema_v222(output: dict) -> dict:
    """Validate Step2 v2.2.2 output schema - strict validation"""
    errors = []
...
def run_step2_with_retry(
    ...
    for attempt in range(max_retries):
        ...
        system_prompt, user_prompt = build_step2_prompt(
            step1_data, project_structure, bp_text, retry
        )
        ...
        try:
            ...
            # Validate schema
            validation = validate_step2_schema_v222(output)
...
"""
        if sec.get("checks") and isinstance(sec["checks"], list):
            ...
            for check in sec["checks"][:5]:
                ...
                md += f"- [{verdict.upper()}] {bucket}: {step1_field}: {reasoning}\n"
            md += "\n"
...
"""
    for result in all_results:
        ...
        if isinstance(eil, list):
            for item in eil:
                if isinstance(item, dict):
                    ...
                    if any(kw in stmt for kw in ["模块", "电控", "系统集成", "集成商", "module", "system", "hardware supplier"]):
        ...
        is_module_or_system = any(
            kw in core_identity
            for kw in ["模块", "电控", "系统集成", "集成商", "改装", "制造", "module", "system", "integration", "hardware supplier"]
        )

        c1_status = "PASS" if (step2_reinforces_module or is_module_or_system) else "FAIL"
        md += f"- **C1**: {c1_status}\n"
        md += f"  - step2_reinforces_module: {step2_reinforces_module}\n"
        md += f"  - is_module_or_system (Step1 core_identity): {is_module_or_system}\n"
        md += f"  - core_identity: {core_identity[:100]}\n"
...
"""
    ...
    for result in all_results:
        ...
        ptype = get_project_type(step1, ps) if is_hardtech_project(step1, ps)[0] else "non-hardtech"
        project_type_map[project] = ptype
    ...
    for result in all_results:
        ...
        is_ht, _ = is_hardtech_project(step1, ps)
        ptype = project_type_map.get(project, "unknown")
        ...
        if isinstance(checks, list):
            tech_ca_count = len([c for c in checks if isinstance(c, dict) and c.get("bucket_key") == "tech_barrier" and c.get("verdict") == "caution"])
            comm_ca_count = len([c for c in checks if isinstance(c, dict) and c.get("bucket_key") == "commercialization" and c.get("verdict") == "caution"])

        if not is_ht:
            md += f"- **{project}**: N/A (非硬科技项目), tech_caution=0, comm_caution=0\n"
            continue

        if ptype in ("tech_rd", "mixed"):
            tech_pass = tech_ca_count >= 1
            comm_pass = comm_ca_count >= 1
            status = "PASS" if (tech_pass and comm_pass) else "FAIL"
            md += f"- **{project}** [{ptype}]: {status}\n"
            md += f"  - tech_barrier caution: {'PASS' if tech_pass else 'FAIL'} ({tech_ca_count}/1 required)\n"
            md += f"  - commercialization caution: {'PASS' if comm_pass else 'FAIL'} ({comm_ca_count}/1 required)\n"
        elif ptype == "system_integration":
            comm_pass = comm_ca_count >= 1
            status = "PASS" if comm_pass else "FAIL"
            md += f"- **{project}** [{ptype}]: {status}\n"
            md += f"  - tech_barrier caution: N/A (集成型，tech_barrier 可为 support)\n"
            md += f"  - commercialization caution: {'PASS' if comm_pass else 'FAIL'} ({comm_ca_count}/1 required)\n"
...
"""
    for result in all_results:
        ...
        if isinstance(checks, list):
            for c in checks:
                if isinstance(c, dict) and c.get("verdict") == "contradict":
                    contradicts += 1
                    contradicts_list.append(c.get("step1_field", "unknown"))

        stance_ok = step1_stance == "cautious_watch"
        no_contradict = contradicts == 0
        a1_status = "PASS" if (stance_ok and no_contradict) else "FAIL"

        md += f"- **A1**: {a1_status}\n"
        md += f"  - step1_stance: {step1_stance} {'PASS' if stance_ok else 'FAIL'}\n"
        md += f"  - contradicts: {contradicts} {'PASS' if no_contradict else 'FAIL'}\n"
...
def run_experiment(projects: list, workspace: str):
    """Run Step2 v2.2.2 external logic validation experiment"""

    print(f"Starting Step2 v2.2.2 experiment...")
    ...
    for project_name in projects:
        ...
        # Detect hard tech
        is_hardtech, keywords = is_hardtech_project(step1_data, project_structure)
        print(f"  - Hard tech detected: {is_hardtech} ({', '.join(keywords[:5])})")
        ...
        output_dir = Path(workspace) / "step2_external_check_v222" / project_name
        ...
        with open(output_dir / "step2_external_check_v2_2_2.json", "w", encoding="utf-8") as f:
        ...
        # Show caution counts for hard tech projects
        if is_hardtech:
            checks = step2_output.get("step1_external_check", {}).get("checks", []) if isinstance(step2_output, dict) else []
            if isinstance(checks, list):
                tech_ca = len([c for c in checks if isinstance(c, dict) and c.get("bucket_key") == "tech_barrier" and c.get("verdict") == "caution"])
                comm_ca = len([c for c in checks if isinstance(c, dict) and c.get("bucket_key") == "commercialization" and c.get("verdict") == "caution"])
                print(f"  - tech_barrier cautions: {tech_ca}, commercialization cautions: {comm_ca}")
    ...
    summary_path = Path(workspace) / "step2_external_check_v222" / "summary.md"
