...
【禁止直接当事实的 BP 声称】（必须在 bp_usage_audit 中追踪）：
- 公司自称的任何头衔/定位（如"全产业链"、"芯片原厂"、"平台"）
- 客户合作关系（"已与XX头部客户合作"）
- 技术能力（"已实现XX技术突破"）
- 市场地位（"国内唯一"、"行业领先"）
- 战略合作（"已与XX企业建立战略合作"）
...
【禁止直接当事实的 BP 声称】
以下 BP 声称不能直接作为事实使用，必须在 bp_usage_audit 中追踪：
- 公司自称的任何头衔/定位（如"全产业链"、"芯片原厂"、"平台"）
- 客户合作关系（"已与XX头部客户合作"）
- 技术能力（"已实现XX技术突破"）
- 市场地位（"国内唯一"、"行业领先"）
...
### 4.4 knowledge_source 分类

每个 external_investment_logic 和 step1_external_check.checks 必须标注知识来源：

| knowledge_source | 说明 | 示例 |
|----------------|------|------|
| `general_bucket` | 来自通用 bucket（general.py） | "技术壁垒需要客户验证" |
| `industry_bucket` | 来自行业增强 bucket（industries/*.py） | "SiC芯片的衬底国产化率低" |
| `llm_common_sense` | 仅来自 LLM 通用常识 | "早期公司收入不稳定" |

**用途**：评估 bucket 是否真的提升了 Step3 的判断质量。
...
```json
{
  ...
  // =========================================
  // 6. BP 使用审计（bp_usage_audit） - v2.2 新增
  // =========================================
  "bp_usage_audit": {
    ...
    "bp_claims_checked": [
      {
        "claim": "BP 中声称的具体内容",
        "used_in": "external_investment_logic[index] | step1_external_check.checks[index]",
        "used_as": "fact | context | not_fact（是否作为事实使用）",
        "note": "说明"
      }
    ],
    "bp_claims_not_used_as_fact": [
      {
        "claim": "BP 中声称但未作为事实使用的内容",
        "reason": "为什么没有直接当事实",
        "treated_as": "待验证 | 包装信号 | 需要创始人确认"
      }
    ],
    "violations": [
      {
        "type": "bp_claim_used_as_fact | new_company_essence | unsupported_conclusion",
        "description": "违规描述",
        "location": "发生在哪个字段/输出中",
...
| bp_usage_audit | 是 | **v2.2 新增**，追踪 BP 使用情况和违规 |
| meta.knowledge_source_breakdown | 是 | **v2.2 新增**，统计知识来源分布 |
...
```json
{
  "hard_constraints": {
    ...
    "must_track_bp_usage": "所有 BP 使用必须进入 bp_usage_audit",
    "bp_forbidden_as_fact": "以下 BP 声称不能直接当事实：自称定位、客户合作、技术能力、市场地位、战略合作",
    "must_label_knowledge_source": "每条 external_investment_logic 和 step1_external_check.checks 必须标注 knowledge_source"
  }
}
```
...
| **输出风格** | 外部视角校验 | BP 内部审计 |
| **BP 使用** | 仅核对 claim | 深度分析 BP 内部 |
| **允许做的事** | reinforce/weaken/needs_verification | 识别包装话术、不一致、夸大 |
| **不允许做的事** | 重读 BP、重新分析项目、产生新结论、把 BP 当事实 | 替代 Step1、下投资结论 |
...
### 7.4 knowledge_source 判定规则

| 场景 | knowledge_source |
|------|-----------------|
| prompt 中 general.py 的通用投资逻辑 | `general_bucket` |
| prompt 中 industries/*.py 的行业增强知识 | `industry_bucket` |
| prompt 中没有，但 LLM 自行推理 | `llm_common_sense` |
...
## 九、附录：v2.1 vs v2.2 差异

| 维度 | v2.1 | v2.2 |
|------|------|------|
| BP 使用审计 | 无 | **新增 bp_usage_audit** |
| knowledge_source | 无 | **每条 external_investment_logic 和 step1_external_check 必须标注** |
| 禁止 BP 声称列表 | 无 | **新增"禁止直接当事实的 BP 声称"** |
| meta 统计 | 无 | **新增 knowledge_source_breakdown** |

---

## 十、附录：v2.0 vs v2.2 完整差异

| 维度 | v2.0 | v2.2 |
|------|------|------|
| external_investment_logic | 泛化的"这类项目..." | **必须绑定 Step1 字段 + 标注 knowledge_source** |
...
| BP 使用审计 | 无 | **bp_usage_audit 追踪所有 BP 使用** |
| 禁止 BP 声称 | 无 | **禁止直接当事实的声称列表** |
| meta | 基础信息 | **新增 new_conclusion_generated + knowledge_source_breakdown** |

---

## 十一、附录：现有 Step3 vs 新 Step3 对比

| 维度 | 旧版 Step3 | 新版 Step3 v2.2 |
|------|-----------|----------------|
| BP 原文定位 | 主要输入 | 仅用于 claim 核对 |
| 判断依据 | BP + 行业逻辑混合 | 纯外部行业/投资逻辑 |
| 输出结构 | bucket_outputs + tensions | external_investment_logic（绑定Step1）+ step1_external_check + bp_usage_audit |
| 核心问题 | "这个 bucket 怎么看" | "这类项目外部怎么看，校验 Step1 的哪些判断" |
| BP 叙事风险 | 高（沉浸式读取） | 低（严格约束 + 审计追踪） |
| 新结论风险 | 中 | **低（有硬约束 + violations 检测）** |
| 知识来源追踪 | 无 | **knowledge_source 标注 + breakdown 统计** |
| 与 Step3B 关系 | 混合 | 明确分工 |
