...
- 哪些判断符合行业常识（support）
- 哪些判断可能需要收缩或加条件（contradict）
- 哪些关键因素在 Step1 中缺失（neutral）

---

【第三部分：转化为决策缺口（供 Step4 使用）】

请输出 3–5 个"必须在投资人会议中确认的问题"。
...
建议将判断写入 `tensions` 或 `step1_adjustment_hints.to_step4`，
也可作为总结段落输出。
...
6. 定位校准结论写入要求（强制执行）：
- tensions 字段必须至少包含一条张力描述，明确写出"当前定位 vs 估值叙事落差"（例如：当前是材料供应商，但BP叙事为平台型公司）。
- step1_adjustment_hints.to_step4 字段必须至少包含一条字符串，明确写出"公司当前定位判断"（例如：该公司当前定位为材料供应商，非平台型公司，建议Step4重点验证平台叙事的真实性）。
